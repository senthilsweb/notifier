...
export GOOS="windows"
export CGO_ENABLED=0
echo go build -ldflags "-X $REPO_NAME/cmd.GitCommit=$GIT_COMMIT -X $REPO_NAME/cmd.Version=$TAG -X $REPO_NAME/cmd.BuildDate=$DATE" -o $DEST$BIN_NAME'windows_386.exe' -v

export GOARCH="amd64"
export GOOS="windows"
export CGO_ENABLED=0
echo go build -ldflags "-X $REPO_NAME/cmd.GitCommit=$GIT_COMMIT -X $REPO_NAME/cmd.Version=$TAG -X $REPO_NAME/cmd.BuildDate=$DATE" -o $DEST$BIN_NAME'windows_amd64.exe' -v


cp server.service.txt $DEST$APP_NAME'.service'
cp server.env $DEST$APP_NAME'.env'
